...
export default function ProductsPage({ onBack, onCreate, onSelectProduct }: ProductsPageProps) {
  ...
  const gridItems = useMemo(() => {
    ...
    const padded: Array<ProductItem | null> = items.slice(0, need)
      ...
      <div className="space-y-10">
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-center">
          ...
          <div className="space-y-2 text-center">
            <h1 className="text-xl font-semibold text-slate-900">상품 관리</h1>
            <p className="text-sm text-slate-500">전체 상품 목록을 확인합니다.</p>
          </div>
          <Button
            type="button"
            className={`justify-self-end h-14 rounded-xl px-8 text-lg font-semibold shadow-sm ${primaryBtn}`}
            onClick={onCreate}
          >
            상품 등록
          </Button>
              ...
              >
                ...
                <div className="mt-6 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className={`${statBox} col-span-2`}>
                      <p className="text-[10px] font-semibold tracking-[0.16em] text-slate-400">
                        재고
                      </p>
                      <p className="mt-2 text-sm font-semibold text-slate-900">
                        {item.stockQuantity.toLocaleString('ko-KR')}
                      </p>
                    </div>
                  </div>
                </div>
